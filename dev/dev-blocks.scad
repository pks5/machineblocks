...
block(baseLayers=1, grid=[2,2], gridOffset=[2,2,0], knobs = false, baseRoundingRadius = [0,0,7.8], baseReliefCut=true);
        ...
        *difference(){
block(grid=[4,4], knobs=false, center=false){

    
   block(grid=[2,2], gridOffset=[0,0,0], knobs=false, center=false,baseCutoutType="none", baseSideAdjustment=1.5);
    
};

 block(grid=[2,2], gridOffset=[2,2,0], baseLayers=3, knobs=false, center=false,baseCutoutType="none", baseSideAdjustment=0.1, alignBottom=false);
 }
 
 
 block(grid=[4,2], baseLayers=3, knobs=true, center=false, align=["start","start","start"], previewRender=false);
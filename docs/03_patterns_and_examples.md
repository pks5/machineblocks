...
This document describes Block File structure, module patterns, and SCAD code examples as used in the MachineBlocks V3 SCAD library. Block Files are SCAD render target artifacts. In the MBML/MBOM workflow, Block Files are generated by the MBOM-to-SCAD compiler — they are not authored manually. The patterns described here (Primitive Wrapper, Semantic Block, Composite Block) reflect the structural logic of the SCAD render target. The compositional concepts — particularly the distinction between simple and composite blocks — carry over into MBML, but MBML authoring and AI-assisted MBML generation are outside the scope of this document.
